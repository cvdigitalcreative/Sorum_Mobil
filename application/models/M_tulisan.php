...
class M_tulisan extends CI_Model{
	...
	function simpan_tulisan($judul,$isi,$harga,$harga_promo,$kategori_id,$kategori_nama,$album_id,$album_nama,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("INSERT into tbl_tulisan(tulisan_judul,tulisan_isi,tulisan_harga,tulisan_harga_promo,tulisan_kategori_id,tulisan_kategori_nama,tulisan_album_id,tulisan_album_nama,tulisan_pengguna_id,tulisan_author,tulisan_gambar,tulisan_slug) values ('$judul','$isi','$harga','$harga_promo','$kategori_id','$kategori_nama','$album_id','$album_nama','$user_id','$user_nama','$gambar','$slug')");
		return $hsl;
	}
	...
	function update_tulisan($tulisan_id,$judul,$isi,$harga,$harga_promo,$kategori_id,$kategori_nama,$album_id,$album_nama,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("UPDATE tbl_tulisan set tulisan_judul='$judul',tulisan_isi='$isi',tulisan_harga='$harga',tulisan_harga_promo='$harga_promo',tulisan_kategori_id='$kategori_id',tulisan_kategori_nama='$kategori_nama',tulisan_album_id='$album_id',tulisan_album_nama='$album_nama',tulisan_pengguna_id='$user_id',tulisan_author='$user_nama',tulisan_gambar='$gambar',tulisan_slug='$slug' where tulisan_id='$tulisan_id'");
		return $hsl;
	}
	function update_tulisan_tanpa_img($tulisan_id,$judul,$isi,$harga,$harga_promo,$kategori_id,$kategori_nama,$album_id,$album_nama,$user_id,$user_nama,$slug){
	...
	function get_tulisan_by_kategori12($kategori_id){
	...
	function get_tulisan_by_kategori($kategori_id){
		$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan where tulisan_kategori_id='$kategori_id' ORDER BY tulisan_id ASC limit 1,1000" );
		return $hsl;
	}

	function get_tulisan_by_kategori_withlimit($kategori_id,$limit){
		$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan where tulisan_kategori_id='$kategori_id' ORDER BY tulisan_id ASC limit $limit" );
		return $hsl;
	}
